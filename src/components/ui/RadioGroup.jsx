...
export function RadioItem({ value, name, selectedValue, onChange, radioColor, children }) {
	...
	return (
		<label className={`flex items-center gap-4 cursor-pointer transition-all`}>
			<input
				type="radio"
				name={name}
				value={value}
			...
			/>
			<span
				className={`w-5 h-5 flex justify-center items-center rounded-full border-2 self-start ${
					isSelected ? `${radioColor} border-current` : "border-gray-400"
				}`}
			>
        {isSelected && (
